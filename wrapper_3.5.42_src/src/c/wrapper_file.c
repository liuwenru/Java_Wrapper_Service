                          ...
                          int logWarningLogLevel)
{
    ...
    do {
        ...
        if (c != NULL) {
            /* First check if line should be skipped and  */
            ...
            if (_tcslen(trimmedBuffer) > 0) {
                if (trimmedBuffer[0] != TEXT('#')) {
                    ...
                    ((_tcsstr(trimmedBuffer, TEXT("#include ")) == trimmedBuffer) || (_tcsstr(trimmedBuffer, TEXT("#include.required ")) == trimmedBuffer))) {
                        ...
                        evaluateEnvironmentVariables(c, expBuffer, MAX_PROPERTY_NAME_VALUE_LENGTH, logWarnings, warnedVarMap, logWarningLogLevel, ignoreVarMap, NULL);
