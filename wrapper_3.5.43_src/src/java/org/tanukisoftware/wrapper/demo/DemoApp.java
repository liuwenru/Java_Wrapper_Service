...
public class DemoApp implements WrapperListener
{
    ...
    private File m_wrapperBin;
    private String m_wrapperBinS;
    ...
    private DemoApp()
    {
        ...
        File wrapperBinDeltaPack;
        File wrapperBinStd;
        // See if the binary exists with delta pack naming.
        if ( WrapperManager.isWindows() )
        {
            wrapperBinDeltaPack = new File( "../bin/" + WrapperManager.generateDetailedNativeBaseName( "wrapper" ) + ".exe" );
            wrapperBinStd = new File( "../bin/wrapper.exe" );
        }
        else
        {
            wrapperBinDeltaPack = new File( "../bin/" + WrapperManager.generateDetailedNativeBaseName( "wrapper" ) );
            wrapperBinStd = new File( "../bin/wrapper" );
        }
        if ( wrapperBinDeltaPack.exists() )
        {
            m_wrapperBin = wrapperBinDeltaPack;
        }
        else if ( wrapperBinStd.exists() )
        {
            m_wrapperBin = wrapperBinStd;
        }
        else
        {
            throw new IllegalStateException( getRes().getString( "Unable to locate Wrapper binary at {0} or {1}", wrapperBinDeltaPack.toString(), wrapperBinStd.toString() ) );
        }
        
        m_wrapperBinS = m_wrapperBin.toString();
        if ( WrapperManager.isWindows() )
        {
            m_wrapperBinS = m_wrapperBinS.replace( '/', '\\' );
        }
        System.out.println( "wrapperBinS=" + m_wrapperBinS );
    ...
    private Process callAction( String action ) throws IOException
    {
        ...
        if ( action.equals( "mail" ) )
        {
            ...
            if ( md.getResult() != 0 )
            {
                ...
                String arg = m_wrapperBinS + " -c " + m_confFile + " wrapper.console.flush=TRUE wrapper.console.format=LPM wrapper.app.parameter.1=start " + md.getEvents()
        ...
        {
            p = Runtime.getRuntime().exec( m_wrapperBinS + " -it " + m_confFile + " wrapper.console.flush=TRUE" );
            // WrapperManager.exec( "sudo ../test/demoapp remove" );
            if ( p != null )
            {
                m_frame.jTabbedPane2.setSelectedIndex( 1 );
        ...
        {
            String arg = m_wrapperBinS + " -c " + m_confFile + " wrapper.console.flush=TRUE wrapper.console.format=LPM wrapper.app.parameter.1=" + action;
        ...
        { 
            ...
            if ( input != null && input.length() > 0 )
            {
                String arg = m_wrapperBinS + " -c " + m_confFile + " wrapper.console.flush=TRUE wrapper.console.format=LPM wrapper.app.parameter.1=start";
                //System.out.println( "calling: " + arg );
                if ( !m_isTestCaseRunning )
                {
                    p = Runtime.getRuntime().exec( arg );
